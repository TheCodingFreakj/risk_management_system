...
# You need to sign up for an API key at https://fred.stlouisfed.org/
FRED_API_KEY = 'd15c7f85662f4cdf7a199e82ac15fed0'

def fetch_market_conditions():
    try:
        fred = Fred(api_key=FRED_API_KEY)
        
        # Fetching GDP growth (annualized percent change)
        gdp_growth = fred.get_series('GDP')[-1]  # Use 'GDP' for nominal GDP data
        
        # Fetching interest rates (10-Year Treasury Constant Maturity Rate as a proxy)
        interest_rate = fred.get_series('DGS10')[-1]  # 'DGS10' is the 10-Year Treasury Constant Maturity Rate
...
def index(request):
    ...
    for ticker, data in all_ticker_data.items():
        ...
        #logger.info(f"Ticker: {ticker}, Daily Returns: {daily_returns_data}, Portfolio Returns: {portfolio_returns_data}")
    
   
    # Raw SQL query to select all records from the stress_scenario table
